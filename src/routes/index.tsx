...
function Dashboard({navigate}: {navigate: (v: View) => void}) {
  ...
  return <>
    ...
    <div className="mt-8 grid gap-6 xl:grid-cols-[1.35fr_1fr]">
      <section><div className="mb-4 flex items-center justify-between"><h2 className="font-display text-lg font-bold">Productivity snapshot</h2><span className="text-xs text-muted-foreground">This week</span></div><div className="grid gap-3 sm:grid-cols-3">{[["12","Items generated","+18%",Sparkles],["3.8h","Time saved","+24%",Clock3],["87%","Tasks completed","+9%",CheckCircle2]].map(([val,label,trend,Icon]) => <div key={label as string} className="surface-card rounded-lg p-5"><div className="flex items-center justify-between"><Icon className="size-4 text-accent-foreground" /><span className="text-xs font-semibold text-success">{trend as string}</span></div><p className="mt-5 font-display text-2xl font-extrabold">{val as string}</p><p className="mt-1 text-xs text-muted-foreground">{label as string}</p></div>)}</div></section>
